...
class zip extends Command
{
    ...
    private function handleCheck() {
        ...
        $commonDirs = array_intersect($this->config[' Dirs'], $this->project['dirs']);
        // ...és ha ez a metszet nem adja vissza a teljes  Dirs-t (a szükséges mappákat),
        // akkor bizony hiányzik valami...
        $dirsDiff = array_diff($this->config[' Dirs'], $commonDirs);
        if (count($dirsDiff) > 0) {
            $error = true;
            $this->io->error([
        ...
        $commonFiles = array_intersect($this->config[' Files'], array_keys($this->project['files']));
        $filesDiff = array_diff($this->config[' Files'], $commonFiles);
